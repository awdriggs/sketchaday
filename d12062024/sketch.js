...
function setPoints() {
  ...
  for(let i = 0; i < numCols; i++){
    ...
    for(let j = 0; j < numRows; j++){
      ...
      // offset = 0;
      // }
      let vector = new Point(i * cellWidth + offset + cellWidth/4, j * cellHeight + cellHeight/2, cellWidth);
  ...
  for(let i = 0; i < points.length; i++){
    ...
    for(let j = 0; j < col.length; j++){
      ...
      //add diagnol springs
      if(j < col.length - 1 && i > 0){
        let above = points[i-1][j+1];
        let spring = new VerletSpring2D(p, above, p.distanceTo(above), strength);
        physics.addSpring(spring);
      }

      if(j > 0 && j < col.length && i > 0){
        let above = points[i-1][j-1];
        let spring = new VerletSpring2D(p, above, p.distanceTo(above), strength);
        physics.addSpring(spring);
