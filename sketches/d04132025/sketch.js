  ...
  draw(){
    for(let i = 0; i < this.count; i++){
      if(this.orientation == "horiziontal"){
        ...
        rect(x, y - (this.stripeHeight * this.offset)/2, this.width, this.stripeHeight * this.offset);
        line(x, y, this.x + this.width, y);
      } else {
        let x = this.x + (i+1) * this.stripeWidth - this.stripeWidth/2;
        let y = this.y;

        rect(x - (this.stripeWidth * this.offset)/2, y, this.stripeWidth * this.offset, this.height);
