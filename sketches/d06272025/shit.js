...
class Agent {
  ...
  update(){
    ...
    let tries = 2;
    while(tries){
      let tryMove = this.doMove();

      if(tryMove == true){
        break;
        
      } else {
        this.type = this.swap(this.type);

        grid[agent.y][agent.x].update(this.type);
        console.log("swap")
        console.log("value after swap", this.type);
        tries--;
      }
