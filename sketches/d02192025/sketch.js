...
function draw() {
  ...
  for(let i = 1; i < rows.length; i++){
    ...
    for(let j = 0; j < row.length - 1; j++){
      ...
      let green = map(dist(x1, y1, x3, y3), 0, rectSize * 2, 0, 255);
      let blue = map(dist(x2, y2, x4, y4), 0, rectSize * 2, 0, 255);
      print(green);
      // let green = map(abs(y1 - x2), 0, rectSize * 2, 0, 255);

      
      //poly shit
      // fill(rows[i][j].c); //color is in the third array position for each line
      fill(shade, green, blue);
      stroke(shade, green, blue);
