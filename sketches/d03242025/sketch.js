...
function setup() {
  ...
  for(let i = 0; i < count + 1; i++){
    ...
    widths.push(map(a, -1, 1, 5, cellSize));
    heights.push(map(b, -1, 1, cellSize, 5));
  }
  ...
  for(let i = 0; i < numRows; i++){
    for(let j = 0; j < numCols; j++){
      ...
      if ((i + j) % 2 == 0) {
        ...
        ellipse(j * cellSize + cellSize/2, i * cellSize + cellSize/2, widths[strokeIndex]);
        // fill("red"); // Black squares
        // ellipse(i * cellSize + cellSize/2, j * cellSize + cellSize/2,  heights[strokeIndex]);
      } else {
        fill("red"); // Black squares
        ellipse(i * cellSize + cellSize/2, j * cellSize + cellSize/2, heights[strokeIndex]);
