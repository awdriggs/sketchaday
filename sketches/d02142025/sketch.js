...
function updateLocationInfo(lat, long){
  reverseGeocoder.getClientLocation({
    latitude: Number(lat),
    longitude: Number(long),
  },function(result) {
    console.log(result);
    polLoc = `${result.city}, ${result.countryCode}`
    // el.innerHTML = loc; //set the html to the lcoation
    let cityText = document.querySelector("#location").innerText = polLoc;
    let latText = document.querySelector("#lat").innerText = lat.toFixed(7); 
    let longText = document.querySelector("#long").innerText = long.toFixed(7); 
  });
