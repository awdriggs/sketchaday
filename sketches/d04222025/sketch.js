let numCols = 10;
let numRows = 10;

let colWidths = [];
let rowHeights = [];
let cells = [];
let colors = [];

let currentCol, currentRow;
let prevPos;

let capture;

function setup() {
  createCanvas(800, 800);
  rectMode(CENTER);


  capture = createCapture(VIDEO);
  capture.size(numCols, numRows);
  capture.hide();

  // Initialize widths and heights equally
  for (let i = 0; i < numCols; i++) {
    colWidths.push(width / numCols);
  }
  for (let i = 0; i < numRows; i++) {
    rowHeights.push(height / numRows);
  }

  // Initialize cells
  for (let i = 0; i < numRows; i++) {
    let row = [];
    for (let j = 0; j < numCols; j++) {
      row.push(new Cell(0, 0, 0, 0)); // positions updated in draw()
    }
    cells.push(row);
  }

  noStroke();
}

function draw() {
  background(100);

  updateColors();

  let y = 0;
  for (let i = 0; i < numRows; i++) {
    let x = 0;
    for (let j = 0; j < numCols; j++) {
      let w = colWidths[j];
      let h = rowHeights[i];
      let cx = x + w / 2;
      let cy = y + h / 2;

      let c = cells[i][j];
      c.x = cx;
      c.y = cy;
      c.w = w;
      c.h = h;

      c.draw();
      x += w;
    }
    y += rowHeights[i];
  }

  fill(0);
  ellipse(mouseX, mouseY, 5, 5);
}

function mousePressed() {
  currentCol = getCol(mouseX);
  currentRow = getRow(mouseY);
  prevPos = createVector(mouseX, mouseY);
}

function mouseDragged(){
  updateGrid();
}

function keyPressed(){
  if(key == "g"){
    saveGif('thumb', floor(random(3, 8)));
  } else if(key == "p"){
    saveCanvas('thumb', "jpg");
  }
}

function getCol(mx) {
  let x = 0;
  for (let j = 0; j < numCols; j++) {
    let w = colWidths[j];
    if (mx >= x && mx < x + w) {
      return j;
    }
    x += w;
  }
  return undefined;
}

function getRow(my) {
  let y = 0;
  for (let i = 0; i < numRows; i++) {
    let h = rowHeights[i];
    if (my >= y && my < y + h) {
      return i;
    }
    y += h;
  }
  return undefined;
}

function updateGrid(){

  let currentPos = createVector(mouseX, mouseY);
  let deltaX = currentPos.x - prevPos.x;
  let deltaY = currentPos.y - prevPos.y;

  // Adjust column widths
  if (currentCol !== undefined) {
    let newWidth = constrain(colWidths[currentCol] + deltaX, 10, width - (numCols - 1) * 10);
    let remaining = width - newWidth;
    let others = numCols - 1;

    if (others > 0 && remaining > 0) {
      let totalOthers = 0;
      for (let j = 0; j < numCols; j++) {
        if (j !== currentCol) totalOthers += colWidths[j];
      }

      for (let j = 0; j < numCols; j++) {
        if (j !== currentCol) {
          if (totalOthers > 0) {
            colWidths[j] = max(10, (colWidths[j] / totalOthers) * remaining);
          } else {
            colWidths[j] = remaining / others;
          }
        }
      }
    }

    colWidths[currentCol] = newWidth;
  }

  // Adjust row heights
  if (currentRow !== undefined) {
    let newHeight = constrain(rowHeights[currentRow] + deltaY, 10, height - (numRows - 1) * 10);
    let remaining = height - newHeight;
    let others = numRows - 1;

    if (others > 0 && remaining > 0) {
      let totalOthers = 0;
      for (let i = 0; i < numRows; i++) {
        if (i !== currentRow) totalOthers += rowHeights[i];
      }

      for (let i = 0; i < numRows; i++) {
        if (i !== currentRow) {
          if (totalOthers > 0) {
            rowHeights[i] = max(10, (rowHeights[i] / totalOthers) * remaining);
          } else {
            rowHeights[i] = remaining / others;
          }
        }
      }
    }

    rowHeights[currentRow] = newHeight;
  }

  prevPos = currentPos;
}

function updateColors(){
  capture.loadPixels();

  for(let i = 0; i < cells.length; i++){
    let row = cells[i];

    for(let j = 0; j < row.length; j++){
      let pIndex = (i * numRows + j) * 4;

      let r = capture.pixels[pIndex];
      let g = capture.pixels[pIndex+1];
      let b = capture.pixels[pIndex+2];

      cells[i][j].color = color(r,g,b);
    }
  }

}

class Cell {
  constructor(x, y, w, h) {
    this.x = x;
    this.y = y;
    this.w = w;
    this.h = h;
    this.color = color(255,255,255); //init to white
  }

  draw() {
    fill(this.color);
    rect(this.x, this.y, this.w, this.h);
  }
}


