...
function setup() {
  ...
  for(let i = 0; i < pattern.length; i++){ //a row
    for(let j = 0; j < pattern[i].length; j++){
      ...
      blocks.push(new Block(x, y, blockSize, blockSize, i+j, config[index].func, config[index].params)); //use i for state, index of starting color
    }
  }
...
function brokenPlates(block, params){
  ...
  let vertices =  [[x,y], [x+w/2, y], [x+w, y], [x+w, y+w/2], [x+w, y+w], [x+w/2, y+w], [x, y+w], [x, y+w/2], [x,y]]
