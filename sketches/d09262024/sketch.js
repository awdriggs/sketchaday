...
function draw() {
  ...
  if(mouseIsPressed){
    ...
    cornerX += cellSize;
    hourGlassVertical(cornerX, cornerY, 6, cellSize);
    hourGlassHorizontal(cornerX, cornerY, 30, cellSize);
  } else {

    // topFilter(cornerX, cornerY, 30, cellSize);
    // leftFilter(cornerX, cornerY, 30, cellSize);
    horiziontalLines(cornerX, cornerY, 30, cellSize);
    ...
    zigzagLines(cornerX, cornerY, 30, cellSize);

    cornerX += cellSize;
    // diagonalRightFull(cornerX, cornerY, 30, cellSize);
    // nestedSquare(cornerX, cornerY, 30, cellSize);
    verticalLines(cornerX, cornerY, 30, cellSize);
  }
}
...
function zigzagLines(cornerX, cornerY, count, cellSize) {
  let offset = cellSize/count;
