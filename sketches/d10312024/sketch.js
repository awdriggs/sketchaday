...
function setup(){
  ...
  let offset = 10;
  for(let i = 0; i < 2; i++){
    cells.push(new Cell(i * frameWidth + offset, height/2 - frameHeight/2 + offset, frameWidth, frameHeight, 255));
  }
  //auto size
  ...
  //   frameWidth = floor(windowWidth/2);// - offset;
  //   frameHeight = frameWidth * 2;
  //   for(let i = 0; i < 2; i++){
...
function draw(){
  ...
  // if(frameCount == 1){
  //   print("saveGif");
  //   saveGif('divisions', numDivisions * 10, { units: "frames"});
  // }

  if(frameCount % 10 == 0 && numDivisions > 0){
    randomDivide();
  }
}
...
function randomDivide(){
  ...
  while(c.h < 30 || c.w < 30){ //random cell is too small, get a new one
