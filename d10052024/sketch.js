...
// let drawingFunctions = [topFilter, bottomFilter, hourGlassVertical, rightFilter, leftFilter, hourGlassHorizontal, nestedSquare, cornerSquare, diagonalRightBottom, diagonalRightTop, diagonalRightFull, diagonalLeftBottom, diagonalLeftTop, diagonalLeftFull, nestedDiamonds,nestedDiamondsInSquare, horizontalLines, verticalLines, zigzagVerticalLines, zigzagHorizontalLines]
let drawingFunctions = [topFilter, bottomFilter, hourGlassVertical, rightFilter, leftFilter, hourGlassHorizontal, nestedSquare, cornerSquare, diagonalRightBottom, diagonalRightTop, diagonalLeftBottom, diagonalLeftTop, nestedDiamonds,]; 
let drawIndex = 0;
...
function draw() {
  ...
  let cellSize = 600;
  let margin = (width - cellSize)/2;
  let cornerX = margin;
  let cornerY = margin;

  // fill('red');
  // rect(cornerX, cornerY, cellSize, cellSize)
  let currentFunction = drawingFunctions[drawIndex];
  grid(cornerX, cornerY, 4, cellSize, currentFunction, 30);
  ...
  if(drawIndex == drawingFunctions.length){
    drawIndex = 0;
    print(frameCount);
  }
}
...
function keyPressed(){
  if(key == "r"){
    saveGif("drawing_system_1", 0.5 * drawingFunctions.length)
  }
}
