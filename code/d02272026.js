...
function draw() {
  ...
  // Horizontals: row by row, connect successive non-null points
  for (let i = 0; i < grid.length; i++) {
    let prev = null;
    for (let j = 0; j < grid[i].length; j++) {
      if (grid[i][j]) {
        if (prev) line(prev.x, prev.y, grid[i][j].x, grid[i][j].y);
        prev = grid[i][j];
      }
    }
  }

  // Verticals: col by col, connect successive non-null points
