...
function setup() {
  ...
  for(let i = 0; i < numOverlays; i++){
    let numRows = floor(random(1, 20));
    let numCols = floor(random(1, 20));
    let diagonal = random() > 0.5 ? true : false;
    let thickness = random(1, 4);
    let randomColor = color(random(0, 255), random(0, 255), random(0, 255));
    let grid = new Grid(0, 0, width, height, numRows, numCols, diagonal, randomColor, thickness);
    grids.push(grid);
  }
...
function draw() {
  ...
  if(update){
    ...
    //choose a random grid from the grids array
    let index = floor(random(0, grids.length));
    //remove it, 
    let grid = grids.splice(index, 1)[0];
    grid.update(x, y);
    grids.push(grid);
    //put it at the end to that it is drawn last later
    //update that grid with the random x/y value
  }
