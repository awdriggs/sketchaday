...
function setup() {
  ...
  spores.push(new Spore(50, 50, color(0, 0, 255)));
}
...
class Spore {
  ...
  spawn(){
    ...
    //top left
    if(this.hLoc - 1 >= 0 && this.vLoc - 1 >= 0 && !this.world[this.vLoc - 1][this.hLoc - 1].fill){
      children.push(new Spore(this.hLoc - 1, this.vLoc - 1, this.color));
    }

    //top right
    if(this.hLoc + 1 < this.world[this.vLoc].length && this.vLoc - 1 >= 0 && !this.world[this.vLoc - 1][this.hLoc + 1].fill){
      children.push(new Spore(this.hLoc + 1, this.vLoc - 1, this.color));
    }


    //bottom left
    if(this.hLoc - 1 >= 0 && this.vLoc + 1 < this.world.length && !this.world[this.vLoc + 1][this.hLoc - 1].fill){
      children.push(new Spore(this.hLoc - 1, this.vLoc + 1, this.color));
    }

    //bottom right
    if(this.hLoc + 1 < this.world[this.vLoc].length - 1 && this.vLoc + 1 < this.world.length  && !this.world[this.vLoc+1][this.hLoc + 1].fill){
      children.push(new Spore(this.hLoc + 1, this.vLoc + 1, this.color));
    }
