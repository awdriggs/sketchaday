...
function reset() {
  ...
  let hOffset = width/numGradients

  for(let i = 0; i < numGradients; i++){
    let chooseColor = color(random(0,255), random(0,255), random(0,255), 10);
    vGradient = new Gradient(i * yOffset, height, 100, chooseColor, 0);
    chooseColor = color(random(0,255), random(0,255), random(0,255), 10);
    hGradient = new Gradient(i * hOffset, width, 100, chooseColor, 1);
    gradients.push(vGradient);
    gradients.push(hGradient);
  }
}
...
class Gradient {
  constructor(y, h, numBars, c, type){
  ...
  draw(c){
    ...
    for(let b of this.bars){
      ...
      if(this.type == 0){
        ...
        rect(0 - width/2, 0 - height/2, b.h, height);
      }
    }
  }
