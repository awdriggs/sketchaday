...
function setup() {
  ...
  globalRadius = 50;
  points = poissonDiskSampling(globalRadius, 30);
  console.log(points);

  // noLoop();
  noStroke();

  currentFill = color(random(0, 255), random(0, 255), random(0, 255));
  fill(currentFill);
...
function draw() {
  ...
  //draw the completed samples
  for(let i = 0; i < completed.length; i++){
    let samples = completed[i];
    fill(samples.color);
    for(let j = 0; j < samples.points.length; j++){
      // ellipse(samples.points[j].x, samples.points[j].y, globalRadius, globalRadius); 
      let p = samples.points[j]; 
      let xIndex = floor(p.x / globalRadius);
      let yIndex = floor(p.y / globalRadius);
      rect(xIndex * globalRadius, yIndex * globalRadius, globalRadius, globalRadius);
    }
  }
  ...
  for(let i = 0; i < index; i++){
    let p = points[i];
    // ellipse(p.x, p.y, globalRadius, globalRadius);
    let xIndex = floor(p.x / globalRadius);
    let yIndex = floor(p.y / globalRadius);
    rect(xIndex * globalRadius, yIndex * globalRadius, globalRadius, globalRadius);
  }
  ...
  // if(frameCount % 10 == 0){
    ...
    if(index == points.length){
      ...
      completed.push({color: currentFill, points: points});
      points = poissonDiskSampling(globalRadius, 30); //reset the points
      index = 0; //reset the index
      currentFill = color(random(0, 255), random(0, 255), random(0, 255)); //reset the fill
      fill(currentFill);
    }
  // }
...
function setup() {
  ...
  points = poissonDiskSampling(50, 30);
  console.log(points);
...
function draw() {
  ...
  for(let frame of finished){
    fill(frame.color);
    for(let frontier of frame.frontiers){
      for(let p of frontier){
        let xIndex = floor(p.x / cellSize);
        let yIndex = floor(p.y / cellSize);
        rect(xIndex * cellSize, yIndex * cellSize, cellSize, cellSize);
      }
    }
  }
  ...
  if(frameCount % 1 == 0){
    ...
    if(frontierIndex >= frontiers.length){
      finished.push({color: fillColor, frontiers: frontiers});
      frontiers = [];
      frontierIndex = 0;
      points = poissonDiskSampling(50, 30);
      fillColor = color(random(0, 255), random(0,255), random(0,255));
...
function poissonDiskSampling(radius, k){
  ...
  // each cell holds at most one point and any two points within 'radius'
  // of each other must be in adjacent cells.
  cellSize = radius / sqrt(N); // radius / sqrt(2)
  let numCols = ceil(width / cellSize) + 1;
  let numRows = ceil(height / cellSize) + 1;
