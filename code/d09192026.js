...
function setup() {
  ...
  for(let i = 0; i < numCols; i++){
    // let boundsY = i * boundsH + margin/2;
      ...
      cols.push(new Looper(random(boundsX, boundsX + boundsW), random(boundsY, boundsY + boundsH), target));
    }


  boundsW = (width - margin);
  boundsH = (height - margin)/numRows;
  boundsX = margin/2;
  for(let i = 0; i < numRows; i++){
    boundsY = i * boundsH + margin/2;
...
//   4. draw even rows
  ...
  for(let i = 0; i < cols.length; i += 2){ //even cols 
    let l = cols[i];
    l.update();
    l.draw();
  }

  for(let i = 1; i < rows.length; i += 2){ //odd rows 
    let l = cols[i];
    l.update();
    l.draw();
  }

  for(let i = 1; i < cols.length; i += 2){ //odd cols 
    let l = cols[i];
    l.update();
    l.draw();
  }

  for(let i = 0; i < rows.length; i += 2){ //even rows 
    let l = cols[i];
    l.update();
    l.draw();
  }
