...
// }
function build(){
  rects = pack(width, height, 4, 100, 100); //runs the square packing tool with w, h, numRects, cellSize, cellSize
  stakes = [];
  for(let r of rects){
...
class Stack {
  ...
  build(cornerX, cornerY, totalWidth, totalHeight){
    ...
    // let w = random(totalWidth * 0.1, width * 0.75);
    // let h = random(height * 0.1, height * 0.75);
    let numLoops = 5;
    // let offset = totalWidth/2/numLoops; //how tight they should be? always the same
    let offset = 20/(800/width); //for dynamic sizing
