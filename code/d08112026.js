...
function draw() {
  ...
  // }
  beginShape();
  curveVertex(points[points.length - 1].loc.x, points[points.length - 1].loc.y); // duplicate last
  for(let p of points){
    p.applyForce();
    curveVertex(p.loc.x, p.loc.y);
  }
  curveVertex(points[0].loc.x, points[0].loc.y); // duplicate first
  curveVertex(points[1].loc.x, points[1].loc.y); // duplicate second
  endShape();
  // for(let p of points){
  //   p.draw();
  //   p.applyForce();
...
class Point {
  ...
  applyForce() {
    ...
    n.mult(0.01 * (width/800)); // perturbation strength
