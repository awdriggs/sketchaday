...
function draw() {
  ...
  for(let a of agents){
    ...
    if(a.growX || a.growY || a.growXNeg || a.growYNeg){
      ...
      newAgents(a.color,  a.startX, a.startY, a.startX + a.w, a.startY + a.h);
      a.alive = false;
    }
  }

  if(lastNumEmpty == emptyCells.length && emptyCells.length != 0){
    // newAgents(255, 0, 0, numCols, numRows);
  } else {
...
function newAgents(targetColor, minX, minY, maxX, maxY){
  ...
  for(let i = 0; i < numAgents; i++){
    ...
    agents.push(new Rectangle(w, h, minX, minY, maxX, maxY, targetColor));
  }
}
