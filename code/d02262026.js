...
//trying for a grid, the vertical lines are fucked, just doing horizontals for now
...
function draw() {
  ...
  //draw the grid
  //each point must be connected to is horizontal neighbor, just look backward
  for(let i = 1; i < compressedGrid.length; i++){
    for(let j = 1; j < compressedGrid[i].length; j++){
      //draw horizontals
      line(compressedGrid[i][j-1].x, compressedGrid[i][j-1].y, compressedGrid[i][j].x, compressedGrid[i][j].y);

      //draw verticals, if there is a matching point, the 2d array is jagged
      // if(compressedGrid[i-1][j]){
      //   line(compressedGrid[i-1][j].x, compressedGrid[i-1][j].y, compressedGrid[i][j].x, compressedGrid[i][j].y);
      // }
    }
  }
...
function getGrid(){
  ...
  //grid has some empty cells, clear them out.
