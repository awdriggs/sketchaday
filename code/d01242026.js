...
function setup() {
  ...
  let pad = width/800 * 50;
  let numCols = floor(width/pad);
  let numRows = floor(height/pad);
  for(let i = 0; i < 8; i++){
    movers.push(new DMover(floor(random(1, numCols)) * pad, floor(random(1, numRows)) * pad));
  }
