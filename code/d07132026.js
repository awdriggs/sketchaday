...
function setup(){
  ...
  numRows = floor(50 * width/800);
  numCols = floor(50 * height/800);
  ...
  while(filledCells.size < Math.min(5000, cells.length)){
    move();
  }
  if(prevLocs.length > 0) oldLines.push({c: currentColor, verts: prevLocs});
  noLoop();
}
...
function draw(){
  ...
  // if(filledCells.size < 5000 && frameCount % 5 == 0){
  //   move();
  // }
...
function move(){
  ...
  for(let i = 0; i < dirs.length; i++){
    if(dirs[i] == "N"){
      let nextX = loc.x;
      let nextY = loc.y - 1;
      if(nextX >= 0 && nextX < numCols && nextY >= 0 && nextY < numRows && !filledCells.has(`${nextX},${nextY}`)){
        possibleDirs.push("N");
      }
    } else if(dirs[i] == "S"){
      let nextX = loc.x;
  ...
  //update the loc
  if(nextMove == "N"){ loc.y -= 1; }
  else if(nextMove == "S"){ loc.y += 1; }
  else if(nextMove == "E"){ loc.x += 1; }
  else if(nextMove == "W"){ loc.x -= 1; }
  filledCells.add(`${loc.x},${loc.y}`);
...
function possibleTurns(){
  if(dir == "N")  return ["N", "E", "W"];
  if(dir == "S")  return ["S", "E", "W"];
  if(dir == "E")  return ["E", "N", "S"];
  if(dir == "W")  return ["W", "N", "S"];
  return ["N", "S", "E", "W"]; // fallback if dir is undefined
}
