...
function init() {
  ...
  while(x < width){
    ...
    // let h = random(minWidth,

    if(width - (x+w) < minWidth){ //if the amount left is less than the minWidth, meaning there can't be another columh
      w = width - x;
    } else if(x + w > width){
      w = width - x;
    }
    ...
    row.push(new Poly(prevX, 0, w, h, fc, gc));
  }

  console.log(row.length);
  grid.push(...row); //push the row items into the grid

  for(let i = 0; i < row.length; i++){
    let x = row[i].cornerX;
    let y = row[i].h; //start at the height of the column
    let w = row[i].w;
    ...
    while(y < height){
      ...
      let fc = color(random(0, 255), random(0, 255), random(0, 255));
      let gc = color(random(0, 255), random(0, 255), random(0, 255));
      grid.push(new Poly(x, prevY, w, h, fc, gc));
