...
function makeShapes(cols, rows){
  ...
  for(let j = 0; j < cols; j++){
    for(let i = 0; i < cols; i++){
      ...
      let x = random(i * xBins + yBins * 0.1, i * xBins + xBins * 0.2) + margin;
      let y = random(j * yBins + yBins * 0.1, j * yBins + yBins * 0.2) + margin;
...
class Poly {
  ...
  buildSquares(sx, sy, w, h){
    ...
    if(type == 0){
      ...
      squares.push({x: sx, y: sy + e + f, w: a, h:g})
      squares.push({x: sx + a, y: sy, w: b, h: h}) //h = e + f + g
      squares.push({x: sx + a + b, y: sy, w: c, h: e})
    } else if(type == 2){
      squares.push({x: sx, y: sy + e + f, w: a, h:g})
      squares.push({x: sx + a, y: sy, w: b, h: h}) //h = e + f + g
      // squares.push({x: sx + a + b, y: sy, w: c, h: e})
