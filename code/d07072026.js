...
function setup(){
  ...
  numRows = floor(20 * width/800);
  numCols = floor(20 * height/800);
...
function draw(){
  ...
  for(let l of oldLines){
    for(let i = 1; i < l.verts.length; i++){
      // line(l.verts[i].x, l.verts[i].y, l.verts[i-1].x + thickness/2, l.verts[i-1].y + thickness/2);
...
function move(){
  ...
  filledCells.add(`${loc.x},${loc.y}`);
  prevLocs.push({x: loc.x * gridSize + gridSize/2 + random(thickness/2, thickness), y: loc.y * gridSize + gridSize/2 + random(thickness/2, thickness)}); //stash the loc before moving
}
