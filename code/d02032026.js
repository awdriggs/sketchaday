...
function reset(){
  ...
  while(mover.complete == false){
   ...
   // Collect all filled cells into a flat array
  for (let r of cells) {
    for (let c of r) {
      if (c.filled) {
  ...
  // Sort by visit order
  occupiedCells.sort((a, b) => a.index - b.index);
...
function draw() {
  ...
  // print("loop");
  beginShape();                                                                                                                                                                                                                            
  for (let c of occupiedCells) {                                                                                                                                                                                                             
    vertex(c.x + c.w / 2, c.y + c.h / 2);                                                                                                                                                                                                  
  }                                                                                                                                                                                                                                        
  // let first = occupiedCells[0]
  // vertex(first.x  + first.w / 2, first.y + first.h / 2);
...
class Mover {
  constructor(sx, sy, env){
    ...
    this.count = 0;
    this.env[this.y][this.x].index = this.count++;
  }
  ...
  move(){
    ...
    while(tries > 0){
      ...
      if(tryX < 1 || tryX >= this.env[this.y].length-1){
        ...
        this.env[this.y][this.x].index = this.count++; //set the index to incremented count
        break;
      } else {
        this.turn();
        tries--;
