...
function setup() {
  ...
  let numCols = floor(50 * width/800);
  let numRows = floor(50 * width/800);
  let cellSize = width / (numCols + 2);

  let startX = (width - (numCols - 1) * cellSize) / 2;
  let startY = (height - (numRows - 1) * cellSize) / 2;

  for(let i = 0; i < numRows; i++){
    let y = i * cellSize + startY;
    for(let j = 0; j < numCols; j++){
      let x = j * cellSize + startX;
      polys.push(new Poly(x, y, cellSize * 0.4, floor(random(5, 10))));
    }
  }
