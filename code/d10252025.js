...
function makeStacks(){
  ...
  for(let x = width/10; x < width - width/10; x += w){
    ...
    if(random() > 0.1){
      let offset = random() > 0.5 ? 0 : h;
      let spacing = random() > 0.5 ? 1 : 2;
      print('spacing', spacing);
      for(let y = height/10; y < height - height/10 - h; y += spacing * h){
        if(spacing % 2 != 0){
          print('swap');
          let temp = c1
          c1 = c2;
        ...
        sliders.push(new Slider(x, y + offset, w, h, c1, c2, 0.75, dir, speed));
      }
    }
