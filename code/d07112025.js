...
function spawnAgent() {
  ...
  if(newLoc){
    let agent = new Agent(newLoc.x, newLoc.y, floor(random(0, 6)));
...
class Cell {
  ...
  draw(){
    ...
    if(this.type == 0){
      ...
      // stroke("red")
      // stroke(this.color)
      // arc(this.x, this.y + this.size, this.size, this.size, HALF_PI * 3, 0)
      ...
      rect(this.x + this.size/2, this.y + this.size/2, this.size/2, this.size/2);
    } else if(this.type == 4){
      // stroke("blue")
  ...
  update(){
    ...
    while(numTries > 0){
      ...
      if(isValid){
        ...
        // let neextType = floor(random(0, 2));
        let nextType = floor(random(0, possibleNextType[moveToTry].length));
        console.log(nextType);
        this.type = possibleNextType[moveToTry][nextType]
