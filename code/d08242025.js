...
function setup() {
  ...
  for(let i = 0; i < numBrushes; i++){
    ...
    print(x);
    let y = i * verticalSpace + random(-verticalSpace/2, verticalSpace/2);

    let rotation = random(-QUARTER_PI, QUARTER_PI);

    let xMin = 0;
    let yMin = i * verticalSpace;
    let xMax = width;
    let yMax = i * verticalSpace + verticalSpace;
    ...
    brushes.push(new Brush(x, y, rotation, xMin, xMax, yMin, yMax, noiseOffset, lineLength, numPoints, 0));
